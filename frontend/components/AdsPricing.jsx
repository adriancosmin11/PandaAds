...
function PricingCard({ name, price, desc, features, color, popular }) {
  ...
  if (isBlue) {
    bgClass =
      "bg-white border-blue-200 shadow-xl shadow-blue-50 ring-2 ring-blue-500 ring-offset-2";
    btnClass =
  ...
  return (
    <div
      className={`relative p-8 rounded-3xl border flex flex-col ${bgClass} transition-transform hover:-translate-y-2`}
    >
      {popular && (
      ...
      <div className="mb-6">
        <h3
          className={`text-sm font-bold tracking-widest uppercase mb-2 ${isBlack ? "text-gray-400" : "text-gray-500"}`}
        >
          {name}
        </h3>
        <div
          className={`text-3xl font-bold mb-1 ${isBlack ? "text-white" : "text-gray-900"}`}
        >
          {price}
        </div>
        <p className={`text-sm ${isBlack ? "text-gray-500" : "text-gray-400"}`}>
          / lună + Buget Ads
        </p>
      </div>

      <p
        className={`text-sm mb-8 pb-8 border-b ${isBlack ? "text-gray-400 border-gray-800" : "text-gray-600 border-gray-100"}`}
      >
        {desc}
      </p>
      ...
      <ul className="space-y-4 mb-8 flex-1">
        {features.map((feat, i) => (
          <li key={i} className="flex items-start gap-3 text-sm">
            <Check
              size={18}
              className={isBlack ? "text-emerald-500" : "text-blue-500"}
            />
            <span className={isBlack ? "text-gray-300" : "text-gray-600"}>
              {feat}
            </span>
          </li>
        ))}
