      ...
      <div className="bg-white rounded-2xl w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50/50 rounded-t-2xl">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Client: <span className="font-semibold text-gray-700">{lead?.nume}</span></p>
          </div>
          <button onClick={onClose} className="p-2 bg-white rounded-full text-gray-400 hover:text-gray-700 hover:bg-gray-100 border border-gray-200 transition-all"><X size={20} /></button>
        </div>
        <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
          <div className="flex items-center justify-between bg-blue-50 p-4 rounded-xl border border-blue-100">
            <div className="flex items-center gap-2"><Edit3 size={18} className="text-blue-600"/><div><p className="text-sm font-bold text-blue-900">Personalizeaza Pretul</p></div></div>
            <label className="relative inline-flex items-center cursor-pointer"><input type="checkbox" className="sr-only peer" checked={isCustomPrice} onChange={() => setIsCustomPrice(!isCustomPrice)}/><div className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-blue-600 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></div></label>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3"><label className="block text-xs font-bold text-gray-500">Pachet Ads</label><select value={selectedAds} onChange={(e) => setSelectedAds(e.target.value)} className="w-full p-3 border rounded-xl bg-gray-50 text-gray-900 font-bold">{Object.keys(ADS_PACKAGES).map(pkg => (<option key={pkg} value={pkg}>{pkg}</option>))}</select><div className={isCustomPrice ? 'opacity-100' : 'opacity-50 pointer-events-none'}><input type="number" value={customAdsPrice} onChange={(e) => setCustomAdsPrice(Number(e.target.value))} className="w-full p-2 border rounded-lg text-gray-900 font-mono"/></div></div>
            <div className="space-y-3"><label className="block text-xs font-bold text-gray-500">Pachet Web</label><select value={selectedWeb} onChange={(e) => setSelectedWeb(e.target.value)} className="w-full p-3 border rounded-xl bg-gray-50 text-gray-900 font-bold">{Object.keys(WEB_PACKAGES).map(pkg => (<option key={pkg} value={pkg}>{pkg}</option>))}</select><div className={isCustomPrice ? 'opacity-100' : 'opacity-50 pointer-events-none'}><input type="number" value={customWebPrice} onChange={(e) => setCustomWebPrice(Number(e.target.value))} className="w-full p-2 border rounded-lg text-gray-900 font-mono"/></div></div>
          </div>
          <div className="bg-red-50 p-4 rounded-xl border border-red-100 flex items-center gap-4"><div className="flex-1"><label className="block text-sm font-bold text-red-900">Discount (EUR)</label><input type="number" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} className="w-full p-2 bg-white border border-red-200 rounded-lg text-red-600 font-bold"/></div><div className="text-right"><span className="block text-xs text-gray-500">Total Final</span><span className="text-2xl font-black text-gray-900">{(isCustomPrice ? (customAdsPrice + customWebPrice) : (ADS_PACKAGES[selectedAds] + WEB_PACKAGES[selectedWeb])) - discount} €</span></div></div>
        </div>
        <div className="p-6 border-t border-gray-100 flex justify-end gap-3 bg-gray-50/50 rounded-b-2xl">
          <button onClick={onClose} className="px-5 py-2.5 text-gray-600 font-bold hover:bg-gray-100 rounded-lg">Anuleaza</button>
